...
export default class Pluralize {
   ...
   */
  checkWord (replaceMap, keepMap, rules, bool) {
    return function (word) {
      var token = word.toLowerCase();
  ...
  plural (word) {
    ...
    let replaceMap=irregularSingles;
    let keepMap=irregularPlurals;
